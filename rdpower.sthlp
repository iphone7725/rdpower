...
{p 4 8}{cmd:rdpower} {hline 2} Power calculations for Regression Discontinuity designs using robust bias-corrected local polynomial inference.{p_end}
...
{p 4 8}{cmd:rdpower} provides power calculations in Regression Discontinuity designs using conventional and robust bias-corrected local polynomial methods.
...
{browse "https://sites.google.com/site/rdpackages/rdpower/Cattaneo-Titiunik-VazquezBare_2018_Stata.pdf": Cattaneo, Titiunik and Gonzalo Vazquez-Bare (2018)}.{p_end}
...
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5)}{p_end}

{p 4 8}Power calculation with covariates{p_end}
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5) covs(population dopen dmidterm)}{p_end}

{p 4 8}Power calculation with user-specified bandwidths{p_end}
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5) h(16 18) b(18 20)}{p_end}

{p 4 8}Power function plot with default options{p_end}
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5) plot}{p_end}

{p 4 8}Power function plot with user-specified range and step{p_end}
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5) plot graph_range(-9 9) graph_step(2)}{p_end}

{p 4 8}Power function plot with user-specified options{p_end}
{p 8 8}{cmd:. rdpower demvoteshfor2 demmv, tau(5) plot graph_range(-9 9) graph_step(2) graph_options(title(Power function) xline(0, lcolor(black) lpattern(dash))  xtitle(tau) ytitle(power) graphregion(fcolor(white)))}{p_end}
...
{p 8 8}Forthcoming, {it:Stata Journal}.{p_end}
...
{p 4 8}Gonzalo Vazquez-Bare, University of California, Santa Barbara.
